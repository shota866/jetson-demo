...
Receives #ctrl messages from operator UIs, integrates a simple vehicle model at
60 Hz, and broadcasts #state messages at up to 30 Hz so that all connected
clients render the same motion. Heartbeats monitor liveness in both
directions. The connection automatically reconnects if Sora drops.
"""
import json
import logging
import math
...
CTRL_HOLD_SEC = 0.2
CTRL_DAMP_SEC = 1.0
auth_logger = logging.getLogger("manager")
...
class VehicleModel:
    """Simple planar vehicle integrator suitable for authoritative playback."""
    ...
    def step(self, ctrl: Optional[ControlSnapshot], dt: float, now: float) -> None:
        ...
        yaw_now = self.yaw + self.wz * dt
        yaw_now = wrap_angle(yaw_now)
...
class ManagerNode:
    ...
    def _connection_loop(self):
        while not self._stop_event.is_set():
            ...
            try:
                ...
                auth_logger.info("connecting to Sora %s", self.signaling_urls)
                conn.connect()
                if not self._connected_event.wait(timeout=10.0):
                    auth_logger.error("Sora connect timeout")
                    conn.disconnect()
                    time.sleep(2.0)
                    self._reconnect_event.set()
                    continue
                self._connection_alive.set()
                auth_logger.info("Sora connected: connection_id=%s", self._connection_id)
                self._disconnected_event.wait()
            except Exception as exc:  # noqa: BLE001
                auth_logger.exception("connection loop error: %s", exc)
                time.sleep(2.0)
            finally:
                with self._conn_lock:
                    if self._conn is not None:
    ...
    def _create_connection(self) -> SoraConnection:
        conn = self._sora.create_connection(
            ...
            data_channels=[
                {"label": self.ctrl_label, "direction": "recvonly", "ordered": True, "max_packet_life_time": None},
                {"label": self.state_label, "direction": "sendonly", "ordered": True, "max_packet_life_time": None},
            ],
        )

        def on_set_offer(raw: str, *, ref=conn):
            self._on_set_offer(ref, raw)

        def on_notify(raw: str, *, ref=conn):
            self._on_notify(ref, raw)

        def on_data_channel(label: str, *, ref=conn):
            self._on_data_channel(ref, label)

        def on_message(label: str, data: bytes, *, ref=conn):
            self._on_message(ref, label, data)

        def on_disconnect(code: SoraSignalingErrorCode, msg: str, *, ref=conn):
    ...
    # --- Sora callbacks ---------------------------------------------------
    def _on_set_offer(self, conn: SoraConnection, raw: str):
        with self._conn_lock:
    ...
    def _on_notify(self, conn: SoraConnection, raw: str):
        with self._conn_lock:
            if conn is not self._conn:
                return
        msg = json.loads(raw)
    ...
    def _on_data_channel(self, conn: SoraConnection, label: str):
        with self._conn_lock:
            if conn is not self._conn:
                return
        if label in self._dc_ready:
            self._dc_ready[label] = True
            auth_logger.info("data channel ready: %s", label)

    def _on_message(self, conn: SoraConnection, label: str, data: bytes):
        with self._conn_lock:
            if conn is not self._conn:
                return
        try:
            ...
            auth_logger.warning("drop malformed json on %s", label)
            return
        ...
        if msg_type == "ctrl" and label == self.ctrl_label:
            ...
            auth_logger.debug("ignore message type=%s label=%s", msg_type, label)

    def _on_disconnect(self, conn: SoraConnection, code: SoraSignalingErrorCode, msg: str):
        with self._conn_lock:
            if conn is not self._conn:
                return
        auth_logger.warning("Sora disconnected: %s %s", code, msg)
        self._connection_alive.clear()
        self._disconnected_event.set()

    # --- message handlers -------------------------------------------------
    def _handle_ctrl(self, msg: Dict[str, object]):
        seq = msg.get("seq")
        if not isinstance(seq, int):
            auth_logger.warning("ctrl without seq: %s", msg)
            return
        cmd = msg.get("cmd") or {}
    ...
    def _handle_heartbeat(self, msg: Dict[str, object]):
    ...
    def _send_state(self, obj: Dict[str, object]) -> None:
        ...
        try:
            ...
            auth_logger.warning("failed to send state: %s", exc)

    def _send_heartbeat(self) -> None:
        if not self._connection_alive.is_set():
            return
        payload = json.dumps(
            {"type": "hb", "role": "server", "t": int(time.time() * 1000.0)}
        ).encode("utf-8")
        with self._conn_lock:
            conn = self._conn
        if not conn:
        ...
        try:
            ...
            auth_logger.debug("heartbeat send failed")
    ...
    def wait_forever(self):
        try:
            ...
            auth_logger.info("interrupt received; stopping")
            self.stop()
...
def load_config(args: argparse.Namespace):
    ...
    if hasattr(args, 'pass') and getattr(args, 'pass') is not None:
        parsed_meta['password'] = getattr(args, 'pass')
    return signaling_urls, channel_id, ctrl_label, state_label, parsed_meta
...
def main():
    ...
    parser.add_argument("--pass", help="Sora room password (overrides SORA_METADATA password)")
    ...
    def handle_signal(_sig, _frame):  # noqa: ANN001
        auth_logger.info("signal received; shutting down")
        node.stop()
